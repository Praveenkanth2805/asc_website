...
export default function NewPortfolio() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    await axios.post("/api/admin/portfolio", form, {
  ...
  return (
    <div className="max-w-lg">
      <h1 className="text-2xl text-gold-400 mb-4">New Portfolio Item</h1>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <button type="submit" className="bg-gold-500 text-black px-6 py-3 rounded font-semibold">Create</button>
      </form>
    </div>
  );
}